...
const Header = ({ toggleSidebar, sidebarOpen }) => {
  ...
  const toggleDrawer = async () => {
    ...
    if (!isDrawerOpen) {
      try {
        ...
        const response = await fetch('https://book-store-backend-sigma-one.vercel.app/contact', {
          method: 'GET',
          headers: {
  ...
  const handleDelete = async (messageId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://book-store-backend-sigma-one.vercel.app/contact/${messageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  return (
    <>
      <div
        className={`w-64 h-screen shadow-lg fixed top-0 left-0 bg-white lg:static lg:w-auto lg:flex lg:flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
      >
        <div className="flex lg:flex-col items-center justify-between w-full p-4 lg:p-0">
          <Link to="/admin" className="mx-4 lg:mx-10">
            <img src={"/logo-removebg.png"} alt="logo" className="w-24 lg:w-55 h-16 lg:h-24" />
          </Link>
        </div>
      ...
      <div className="flex items-center justify-between fixed top-4 right-4 w-full lg:w-auto px-5 bg-white lg:bg-transparent">
        <button onClick={toggleSidebar} className="lg:hidden btn btn-square btn-ghost">
